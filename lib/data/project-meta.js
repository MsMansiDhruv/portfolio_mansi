...
export const PROJECT_META = {
  "project-amc-datalake-solution": {
    ...
    cardTitle: "Legacy Data Modernization",
...
export function getProjectFlow(meta) {
  if (meta?.architectureLayers?.length) return meta.architectureLayers;
  if (meta?.responsibilities?.length) return meta.responsibilities.slice(0, 4);
  return (meta?.tech || []).slice(0, 4);
}
