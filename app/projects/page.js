    ...
    <div className="space-y-16">
      <Reveal>
        <header className="max-w-2xl">
          ...
          <h1 className="mt-4 text-4xl font-semibold tracking-tight text-slate-950 dark:text-white">Projects</h1>
        ...
        <Reveal delay={0.05}>
          <HoverLift>
            <Link
              ...
              className="group grid overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950 lg:grid-cols-[1.2fr_0.8fr]"
            >
              <div className="border-b border-slate-200 p-8 dark:border-slate-800 lg:border-b-0 lg:border-r">
                ...
                <h2 className="mt-2 text-3xl font-semibold text-slate-950 dark:text-white">{featured.title}</h2>
                <p className="mt-2 text-xs text-slate-500">{(featured.tech || []).join(" · ")}</p>
      ...
      <div>
        <input
          ...
          className="mb-8 w-full max-w-sm rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm dark:border-slate-800 dark:bg-slate-950"
          ...
          {others.map((p, i) => {
            ...
            return (
              <Reveal key={p.slug} delay={0.03 * i}>
                <li>
                  <Link href={`/projects/${p.slug}`} className="group flex flex-col gap-2 py-5 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <p className="text-xs text-slate-500">{p.category}</p>
                      <h3 className="text-lg font-semibold text-slate-950 dark:text-white">{p.title}</h3>
                      <p className="mt-1 text-xs text-slate-500">{(p.tech || []).join(" · ")}</p>
